...
## Implemented Tasks
- `origination/task-01` (existing): clean PAYG prime approval (PO -> UW -> APPROVE)
- `origination/task-02`: PO-only missing mandatory docs -> `REQUEST_FURTHER_INFO`
- `origination/task-03`: PAYG near-prime + DTI > 6.0x -> PO -> CM -> `DECLINE`
- `origination/task-04`: PAYG sub-prime (<580) -> PO -> CM -> `DECLINE`
- `origination/task-05`: self-employed DTI hard decline (cloned from task-02 scenario under origination naming)
- `origination/task-06`: DVS mismatch + thin file fraud hold (cloned from task-05 scenario under origination naming)
